# AllInOne.jl

[![Julia 1.12+](https://img.shields.io/badge/Julia-1.12+-blue.svg?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BSnVsaWEgZG90cywgY29weXJpZ2h0IDIwMTItMjAyMiBTdGVmYW4gS2FycGluc2tpLiBDQyBCWS1OQy1TQSA0LjAuIGh0dHBzOi8vZ2l0aHViLmNvbS9KdWxpYUxhbmcvanVsaWEtbG9nby1ncmFwaGljczwvZGVzYz48L3N2Zz4%3D)](https://julialang.org/downloads/)
[![Colab: open](https://badgen.net/static/Colab/open/007ec6?icon=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%%%2BPC9zdmc%2B&iconWidth=22)](https://colab.research.google.com/github/ohno/AllInOne.jl/blob/main/examples/AllInOne.ipynb)
[![Stable](https://img.shields.io/badge/docs-stable-blue.svg)](https://ohno.github.io/AllInOne.jl/stable/)
[![Dev](https://img.shields.io/badge/docs-dev-blue.svg)](https://ohno.github.io/AllInOne.jl/dev/)
[![Citation](https://img.shields.io/badge/citation-BibTeX-778899)](CITATION.bib)
[![license](https://img.shields.io/github/license/ohno/AllInOne.jl?label=license)](LICENSE)
[![code style: runic](https://img.shields.io/badge/code_style-%E1%9A%B1%E1%9A%A2%E1%9A%BE%E1%9B%81%E1%9A%B2-black)](https://github.com/fredrikekre/Runic.jl)
[![contributer's guide: ColPrac](https://img.shields.io/badge/contributer%27s%20guide-ColPrac-blueviolet)](https://github.com/SciML/ColPrac)
[![CI](https://github.com/ohno/AllInOne.jl/actions/workflows/CI.yml/badge.svg?branch=main)](https://github.com/ohno/AllInOne.jl/actions/workflows/CI.yml?query=branch%3Amain)
[![coverage](https://codecov.io/gh/ohno/AllInOne.jl/branch/main/graph/badge.svg)](https://codecov.io/gh/ohno/AllInOne.jl)
[![Aqua](https://img.shields.io/github/actions/workflow/status/ohno/AllInOne.jl/Aqua.yml?branch=main&event=push&label=Aqua&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%2CPHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCA3OS4zNzUgNTguNjc0Ij48ZyBzdHJva2U9IiNmNWY1ZjUiIHN0cm9rZS13aWR0aD0iNC44ODciPjxjaXJjbGUgZmlsbD0iIzM4OTgyNiIgY3g9IjM5LjY4OCIgY3k9IjIzLjk5MiIgcj0iMjEuNTQ5Ii8%2BPGNpcmNsZSBmaWxsPSIjY2IzYzMzIiBjeD0iMTcuNDQzIiBjeT0iNDEuMjMxIiByPSIxNSIvPjxjaXJjbGUgZmlsbD0iIzk1NThiMiIgY3g9IjYxLjkzMiIgY3k9IjQxLjIzMSIgcj0iMTUiLz48L2c%2BPC9zdmc%2B)](https://github.com/ohno/AllInOne.jl/actions/workflows/Aqua.yml?query=branch%3Amain)
[![JET](https://img.shields.io/github/actions/workflow/status/ohno/AllInOne.jl/JET.yml?branch=main&event=push&label=%F0%9F%9B%A9%EF%B8%8F%20JET)](https://github.com/ohno/AllInOne.jl/actions/workflows/JET.yml?query=branch%3Amain)

PkgFactory all-in-one template integration test

## Quick Start

Run the following command in the Julia REPL or a notebook:

```julia
import Pkg; Pkg.add(url="https://github.com/ohno/AllInOne.jl.git")
```

After installation, load the package and verify it works:

```julia
julia> import AllInOne; AllInOne.hello()
"Hello, World!"
```

## Documentation

- Home: https://ohno.github.io/AllInOne.jl
- User Guide: https://ohno.github.io/AllInOne.jl/dev/user
- Developer Guide: https://ohno.github.io/AllInOne.jl/dev/developer
- API Reference: https://ohno.github.io/AllInOne.jl/dev/api
