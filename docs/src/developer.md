...
If you are planning significant changes, open an [issue](https://github.com/ohno/AllInOne.jl/issues) first. The [ColPrac](https://github.com/SciML/ColPrac) guidelines are recommended. For Julia package development basics, see:
...
1. Fork the [repository](https://github.com/ohno/AllInOne.jl) on GitHub.
2. Clone the forked repository. Replace `xxxxxx` with your GitHub username.
   ```sh
   git clone https://github.com/xxxxxx/AllInOne.jl.git
   cd AllInOne.jl
   ```
...
2. Start an interactive session with [Revise.jl](https://github.com/timholy/Revise.jl).
   ```sh
   julia --startup-file=no -i -e 'using Revise; import Pkg; Pkg.activate("."); using AllInOne'
   ```
...
- the version in [Project.toml](https://github.com/ohno/AllInOne.jl/blob/main/Project.toml)
- the version, year, and month in [CITATION.bib](https://github.com/ohno/AllInOne.jl/blob/main/CITATION.bib)
